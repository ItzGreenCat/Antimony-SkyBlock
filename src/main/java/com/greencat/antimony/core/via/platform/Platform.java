...
import com.greencat.antimony.utils.FutureTaskId;
...
public class Platform implements ViaPlatform<UUID> {
    ...
    public FutureTaskId runAsync(Runnable runnable) {
        return new FutureTaskId(CompletableFuture
                .runAsync(runnable, Antimony.getAsyncExecutor())
    ...
    public FutureTaskId runSync(Runnable runnable) {
        return new FutureTaskId(Antimony.getEventLoop().submit(runnable).addListener(errorLogger()));
    }

    @Override
    public PlatformTask runSync(Runnable runnable, long ticks) {
        return new FutureTaskId(Antimony.getEventLoop().schedule(() -> runSync(runnable), ticks *
                50, TimeUnit.MILLISECONDS).addListener(errorLogger()));
    }

    @Override
    public PlatformTask runRepeatingSync(Runnable runnable, long ticks) {
         return new FutureTaskId(Antimony.getEventLoop().scheduleAtFixedRate(() -> runSync(runnable),
    ...
    @Override
    public boolean hasPlugin(String var1){return false;}
}
